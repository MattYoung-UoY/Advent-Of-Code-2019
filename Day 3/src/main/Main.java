...
public class Main {
	...
	private static List<WireSegment> wire1, wire2;
	...
	public static void main(String[] args) {
		...
		int minIntersect = intersections.get(0).getDistance();
		
		for(Point intersect: intersections) {
			minIntersect = Math.min(minIntersect, intersect.getDistance());
		}
		
		System.out.println(minIntersect);
	}

	private static List<Point> getIntersections(List<WireSegment> wire1, List<WireSegment> wire2) {
		List<Point> res = new ArrayList<Point>();

		for (WireSegment seg1 : wire1) {
			for (WireSegment seg2 : wire2) {
				// seg1 is vertical
				if (seg1.getStart().getX() == seg1.getEnd().getX()) {
					// seg2 is horizontal
					if (seg2.getStart().getY() == seg2.getEnd().getY()) {
						// seg1 is vertical and seg2 is horizontal

						int x = seg1.getStart().getX(), y = seg2.getStart().getY();

						int minX = Math.min(seg2.getStart().getX(), seg2.getEnd().getX());
						int maxX = Math.max(seg2.getStart().getX(), seg2.getEnd().getX());

						int minY = Math.min(seg1.getStart().getY(), seg1.getEnd().getY());
						int maxY = Math.max(seg1.getStart().getY(), seg1.getEnd().getY());

						if (minX <= x && maxX >= x) {
							if (minY <= y && maxY >= y) {
								int xIntDist = Math.abs(seg2.getStart().getX() - x),
										yIntDist = Math.abs(seg1.getStart().getY() - y);
								int intersectDist = seg1.getStart().getDistance() + yIntDist
										+ seg2.getStart().getDistance() + xIntDist;

								res.add(new Point(x, y, intersectDist));
							}
						}
				...
				// seg1 is horizontal
				if (seg1.getStart().getY() == seg1.getEnd().getY()) {
					// seg2 is vertical
					if (seg2.getStart().getX() == seg2.getEnd().getX()) {
						// seg1 is horizontal and seg2 is vertical

						int x = seg2.getStart().getX(), y = seg1.getStart().getY();

						int minX = Math.min(seg1.getStart().getX(), seg1.getEnd().getX());
						int maxX = Math.max(seg1.getStart().getX(), seg1.getEnd().getX());

						int minY = Math.min(seg2.getStart().getY(), seg2.getEnd().getY());
						int maxY = Math.max(seg2.getStart().getY(), seg2.getEnd().getY());

						if (minX <= x && maxX >= x) {
							if (minY <= y && maxY >= y) {
								int xIntDist = Math.abs(seg1.getStart().getX() - x),
										yIntDist = Math.abs(seg2.getStart().getY() - y);
								int intersectDist = seg2.getStart().getDistance() + yIntDist
										+ seg1.getStart().getDistance() + xIntDist;
	...
	private static List<WireSegment> getWire(int index) {
		List<WireSegment> res = new ArrayList<WireSegment>();

		String[] segments = input[index].split(",");

		Point start = new Point(0, 0, 0), end = new Point(0, 0, 0);

		for (String segment : segments) {
			int dist = Integer.parseInt(segment.subSequence(1, segment.length()).toString());

			char dir = segment.charAt(0);
			switch (dir) {
			case 'U':
				end.setY(start.getY() + dist);
				break;
			case 'D':
				end.setY(start.getY() - dist);
				break;
			case 'L':
				end.setX(start.getX() - dist);
				break;
			case 'R':
				end.setX(start.getX() + dist);
				break;
			...
			end.setDistance(end.getDistance() + dist);

			res.add(new WireSegment(new Point(start.getX(), start.getY(), start.getDistance()),
					new Point(end.getX(), end.getY(), end.getDistance())));

			start.setX(end.getX());
			start.setY(end.getY());
			start.setDistance(end.getDistance());
		}
